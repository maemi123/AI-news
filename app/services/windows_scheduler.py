...
class WindowsTaskSchedulerService:
    ...
    def _build_task_action(self) -> tuple[str, str]:
        venv_python = self.project_root / '.venv' / 'Scripts' / 'python.exe'
        if venv_python.exists():
            return str(venv_python), '-m app.run_scheduled_job'
        return 'py.exe', '-3 -m app.run_scheduled_job'
    ...
    def _query_task_status_sync(self) -> WindowsTaskStatus:
        ...
        if not task_stdout:
            return WindowsTaskStatus(registered=False, executor_path=self.resolve_python_command())
    ...
    def _create_or_update_task_sync(self, *, hour: int, minute: int) -> None:
        self._ensure_supported()
        execute, arguments = self._build_task_action()
        command = (
        ...
        if result.returncode != 0:
            raise WindowsTaskSchedulerError((result.stderr or result.stdout or 'Failed to create Windows task.').strip())
    ...
    def _delete_task_sync(self) -> None:
        ...
        if result.returncode != 0:
            raise WindowsTaskSchedulerError((result.stderr or result.stdout or 'Failed to delete Windows task.').strip())
